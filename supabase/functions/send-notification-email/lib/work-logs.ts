...
export async function fetchWorkLogExpenseRow(
  admin: SupabaseClient,
  expenseId: number
...
export async function fetchWorkLogMaterialRow(
  admin: SupabaseClient,
  materialId: number
): Promise<Record<string, any> | null> {
...
export function formatBatchedEventDetailLines(ev: {
  ...
  if (table === "work_log_entries") {
    ...
    const sortKey = `${normalize(record.date_from)}|${normalize(record.time_from)}|${normalize(record.details)}|${normalize(
      record.desc_type
    )}`;
    ...
    if (op === "UPDATE") {
      ...
      for (const [key, diff] of Object.entries(changes || {})) {
        if (diff && typeof diff === "object") {
          if ("before" in diff) beforeRec[key] = (diff as any).before;
          if ("after" in diff) afterRec[key] = (diff as any).after;
        }
      }

      const beforeLine = formatEntryLine(beforeRec as WorkLogEntrySummary);
  ...
  if (table === "work_log_persons" || table === "work_log_entry_persons") {
    ...
    if (op === "INSERT") push(`${name} 추가됨`, "add", "참가자", sortKey);
    else if (op === "UPDATE") build(personChangeLabels).forEach((l) => push(l, "update", "참가자"));
    else if (op === "DELETE") push(`${name} → 삭제됨`, "delete", "참가자", sortKey);
    return lines;
  }
  ...
  if (table === "work_log_receipt") {
    ...
    const amount = record.amount != null ? Number(record.amount).toLocaleString("ko-KR") + "원" : "-";
    const date = formatKoreanDate(record.receipt_date || record.date);
    const line = `${date || "-"} | ${vendor} | ${amount}`;
    const sortKey = `${normalize(record.receipt_date || record.date)}|${record.amount ?? ""}|${vendor}`;
    if (op === "INSERT") push(`${line} 추가됨`, "add", "영수증", sortKey);
    else if (op === "UPDATE") build(receiptChangeLabels).forEach((l) => push(l, "update", "영수증"));
    else if (op === "DELETE") push(`${line} → 삭제됨`, "delete", "영수증", sortKey);
    return lines;
  }
...
export async function buildBatchedReportEmail(
  ...
  // ✅ 0) 전체 삭제(=work_logs DELETE 포함)면 메일 만들지 않음
  const hasWorkLogDelete = Array.isArray(events)
    ? events.some((ev: any) => ev?.table === "work_logs" && String(ev?.operation || "").toUpperCase() === "DELETE")
    : false;
  if (hasWorkLogDelete) return { subject: "", text: "", html: "" };

  // ✅ 0-1) 전체 삭제(캐스케이드) 패턴: (전부 DELETE) + (여러 테이블/삭제량 큼)
  const ops = Array.isArray(events) ? events.map((ev: any) => String(ev?.operation || "").toUpperCase()) : [];
  const allDeleteOnly = ops.length > 0 && ops.every((op: string) => op === "DELETE");

  const tableSet = new Set((events || []).map((ev: any) => String(ev?.table || "")));
  const distinctTables = tableSet.size;

  const deleteCountEntries = (events || []).filter(
  ...
  try {
    const { data: wl } = await admin.from("work_logs").select("id").eq("id", workLogId).maybeSingle();
    if (!wl) return { subject: "", text: "", html: "" };
  } catch {
    // 체크 실패 시엔 계속 진행
  }
  ...
  const htmlDetails =
    ...
    (!isCreate
      ? changeLines.length > 0
        ? htmlSections
        : `<ul style="margin:8px 0 0 18px;list-style:disc;padding-left:20px;"><li style="margin:4px 0;color:#6b7280;">(변경 사항 없음)</li></ul>`
      : "");

  const action =
    Number.isFinite(workLogId) && workLogId > 0
      ? `<div style="margin-top:22px;"><a href="${escapeHtml(
          `${REPORT_BASE_URL}/report/${workLogId}`
        )}" target="_blank" rel="noopener noreferrer" style="display:inline-block;background:#111827;color:#ffffff;text-decoration:none;padding:12px 18px;border-radius:999px;font-size:14px;font-weight:700;letter-spacing:0.02em;">보고서 바로가기</a></div>`
      : "";
...
> {
  ...
  if (table === "work_log_entries") {
    ...
    if (normalize(record.move_from) || normalize(record.move_to))
      baseDetails.push(`이동: ${normalize(record.move_from) || "-"} → ${normalize(record.move_to) || "-"}`);

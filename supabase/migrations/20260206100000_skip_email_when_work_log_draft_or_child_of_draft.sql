-- 출장 보고서 임시저장 시 이메일 발송하지 않기: 부모가 draft인 자식(work_log_entries 등) 변경 시에도 스킵
...
begin
  -- Skip drafts for work_logs (임시저장 시 이메일 발송 안 함)
  ...
  -- Skip when parent work_log is draft (작업 일지/지출/자재 추가·수정이어도 보고서가 아직 임시저장이면 이메일 안 함)
  ...
  if (tg_op = 'UPDATE') then
    ...
    -- 임시저장 → 제출 전환 시 항상 이메일 발송 (changes가 비어 있어도 발송)

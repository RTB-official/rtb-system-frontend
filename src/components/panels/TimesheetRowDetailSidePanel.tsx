...
interface TimesheetRowDetailSidePanelProps {
    ...
    manualBillableSplitHoursByEntryId?: Record<
        number,
        {
...
const getEntryStartTime = (
    entry: Pick<
        TimesheetRowDetailSidePanelProps["sourceEntries"][number],
        "dateFrom" | "timeFrom"
    >
): number | null => {
...
export default function TimesheetRowDetailSidePanel({
    ...
    const renderChargeLabelContent = (entry: PanelEntry, chargeLabel: string) => {
        const splitManual = manualBillableSplitHoursByEntryId[entry.id];
        if (entry.descType !== "작업" || !splitManual) {
            return chargeLabel;
        }

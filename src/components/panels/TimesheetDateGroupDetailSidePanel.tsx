...
import EntryDescriptionWithOtherLineBadges from "../common/EntryDescriptionWithOtherLineBadges";
import {
...
interface TimesheetDateGroupDetailSidePanelProps {
    ...
    /**
     * 엔트리 단위: 해당 엔트리 비고에서 Skilled Fitter로 표시할 참여자(엔트리별 지정이 있으면 그 기준만 볼드).
     */
...
export default function TimesheetDateGroupDetailSidePanel({
    ...
    const existingPersonNamesOnDate = useMemo(() => {
        ...
        return sortPeopleByRoleAndJoinDate(order, staffSortMeta);
    }, [fullGroupEntries, dateYmdForPersonnelModal, staffSortMeta]);

    const replacementPersonNames = useMemo(() => {
        const onDate = new Set(existingPersonNamesOnDate);
        return sortPeopleByRoleAndJoinDate(
            invoiceTimesheetPeople.filter((p) => p && !onDate.has(p)),
            staffSortMeta
        );
    }, [invoiceTimesheetPeople, existingPersonNamesOnDate, staffSortMeta]);
    ...
    const getTravelJourneyKey = (entry: TimesheetSourceEntryData): string =>
        [
    ...
    const groupTravelEntriesByJourney = (
        ...
        const journeys: TimesheetSourceEntryData[][] = [];
    ...
    const getFixedTravelHoursByEntryId = (
        ...
        groupTravelEntriesByJourney(entries).forEach((journey) => {
            ...
            const departsFromHome =
                normalizeLocationName(getTravelEntryOrigin(journey[0])) ===
                "\uC790\uD0DD";
            ...
            fillOrder.forEach((item, index) => {
                ...
                allocationByEntryId.set(item.id, roundHours(Math.max(0, allocated)));
    ...
    const summarizeTravelEntries = (
        ...
        const hours = useFixedHomeHours
            ? sumFixedTravelHours(entries)
            : roundHours(
                  entries.reduce(
                      (sum, item) => sum + calculateRawTravelHours(item),
                      0
                  )
              );
    ...
    const getTravelChargeResultForPerson = (
        ...
        if (override?.target === "home" && fixedHours > 0) {
            if (entry.clientHomeTravelSpilloverOf != null) {
                return {
                    hours: roundHours(calculateRawTravelHours(entry)),
                    kind: "home",
                };
            }

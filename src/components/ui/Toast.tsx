...
function Toast({ toast, onClose, offset = 0 }: ToastProps) {
    ...
    return createPortal(
        ...
        >
            <div className="bg-gray-800/90 rounded-lg shadow-2xl min-w-[260px] max-w-[500px] px-5 py-4 flex items-center gap-3 relative overflow-hidden">
                {/* 진행 바 (배경) */}
                ...
                >
                    {toast.type === "success" && (
                        <IconCheckmark className="w-4 h-4 text-white" />
                    )}
                    {toast.type === "error" && (
                        <IconError className="w-4 h-4 text-white" />
                    )}
                ...
                {/* 메시지 */}
                <p className="flex-1 text-[15px] font-regular text-white">
                    {toast.message}
                </p>
            </div>
        </div>,

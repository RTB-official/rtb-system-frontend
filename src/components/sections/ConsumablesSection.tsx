...
import { useWorkReportStore, MATERIALS, MATERIAL_UNITS } from '../../store/workReportStore';
  ...
  return (
    <SectionCard title="소모 자재">
      <div className="flex flex-col gap-5">
        ...
        {/* 수량 */}
        <div className="flex flex-col gap-2">
          <label className="font-medium text-[14px] text-[#101828]">수량</label>
            ...
            <div className="flex flex-wrap gap-2 p-4 border border-dashed border-[#e5e7eb] rounded-xl min-h-[60px]">
              {materials.map((item) => (
                <Chip
                  ...
                  variant="tag"
                  onRemove={() => {
                    if (confirm('삭제하시겠습니까?')) removeMaterial(item.id);
                  }}
                >
                  {formatLabel(item.name, item.qty, item.unit)}
                </Chip>
              ))}
            </div>
          </div>

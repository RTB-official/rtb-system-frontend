...
export default function WorkerSection() {
  ...
  // 직급별 그룹 (인원이 있는 것만 표시)
  const staffGroups = Object.entries(STAFF_DATA).filter(([, names]) => names.length > 0);
      ...
      <div className="flex flex-col gap-4">
          ...
          >
            ...
            <div className="flex gap-2 flex-wrap">
              {names.map((name) => (
                <Chip
                  key={name}
                  variant={workers.includes(name) ? 'selected' : 'default'}
                  onClick={() => workers.includes(name) ? removeWorker(name) : addWorker(name)}
                >
                  {name}
                </Chip>
              ))}
        ...
        <div className="border border-[#e5e7eb] rounded-lg p-4 flex flex-col gap-2 bg-white">
          ...
          <div className="flex gap-2 flex-wrap min-h-[48px]">
            {workers.length === 0 ? (
              <p className="text-[#99a1af] text-sm">위에서 작업자를 선택해주세요</p>
            ) : (
              workers.map((worker) => (
                <Chip
                  key={worker}
                  ...
                  draggable
                  onDragStart={(e) => e.dataTransfer.setData('text/plain', worker)}
                >
                  {worker}
                </Chip>
              ))

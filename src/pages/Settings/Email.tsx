...
export default function EmailNotificationPage() {
  ...
  const loadUserContext = useCallback(async () => {
    try {
      ...
      const canSee = myProfile?.department === "공무팀" && myProfile?.role === "admin";
      ...
      console.error(e);
      showError(e?.message ?? "사용자 정보 로드 실패");
      return null;
    }
  }, [showError]);

  // ===== 이메일 알림 설정 로드 (email view 전용) =====
  ...
  }, [loadUserContext, showError]);

  // 사용자 컨텍스트 및 이메일 설정 로드
  useEffect(() => {
    loadUserContext();
    loadEmailPrefs();
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <main className="flex-1 overflow-auto pt-8 pb-20 px-4 sm:px-6 lg:px-10 bg-gray-50">
          ...
          {loading ? (
            ...
            <div className="w-full max-w-none grid grid-cols-1 lg:grid-cols-[1fr_360px] gap-5">
              {/* ✅ 좌측 컬럼 (이메일 추가 + 이메일 목록) */}
              <div className="flex flex-col gap-5">
                ...
                <div className="bg-white border border-gray-200 rounded-2xl p-6">
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      <h2 className="text-[18px] font-bold text-gray-900">이메일 추가</h2>
                  ...
                  <div className="mt-5 grid grid-cols-1 lg:grid-cols-[1fr_auto] gap-4 items-start">
                    {/* 이메일 입력 */}
                    <div>
                      <div className="text-[13px] font-medium text-gray-700 mb-2">이메일</div>
                      <Input
                        type="email"
                        value={newEmail}
                  ...
                  {/* 타입 선택 */}
                  <div className="mt-4">
                    <div className="text-[13px] font-medium text-gray-700 mb-2">받을 알림 유형</div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                      {(Object.keys(TYPE_LABEL) as NotiType[]).map((k) => (
                        ...
                        >
                          ...
                          />
                          <div className="text-[14px] font-semibold text-gray-900">{TYPE_LABEL[k]}</div>
                        </label>
                      ))}
                    </div>
                  </div>
                ...
                <div className="bg-white border border-gray-200 rounded-2xl p-6">
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      <h2 className="text-[18px] font-bold text-gray-900">이메일 목록</h2>
                      <p className="text-[13px] text-gray-500 mt-1"></p>
                    </div>
                    <div className="text-[12px] text-gray-500">{list.length}개</div>
                  </div>

                  {list.length === 0 ? (
                    <div className="py-10 text-center text-gray-400">등록된 이메일이 없습니다.</div>
                  ) : (
                        ...
                        >
                          <div className="flex items-start justify-between gap-3">
                            <div className="min-w-0">
                              <div className="text-[15px] font-bold text-gray-900 truncate">{item.email}</div>
                              <div className="text-[12px] text-gray-500 mt-1">{item.enabled ? "" : ""}</div>
                            </div>
              ...
              {canSeeAdminList ? (
                <div className="bg-white border border-gray-200 rounded-2xl p-6 h-fit">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <h2 className="text-[18px] font-bold text-gray-900">공무팀</h2>
                      <p className="text-[13px] text-gray-500 mt-1"></p>
                    </div>
                    <div className="text-[12px] text-gray-500">{adminMembers.length}명</div>
                  </div>

                  {adminMembersLoading ? (
                    <div className="mt-6 text-sm text-gray-500">로딩 중...</div>
                  ) : adminMembers.length === 0 ? (
                    <div className="mt-6 text-sm text-gray-400">표시할 명단이 없습니다.</div>
                  ) : (
                    <div className="mt-5 border border-gray-200 rounded-2xl overflow-hidden bg-white">
                      <div className="text-[13px] font-semibold text-gray-700 text-right"></div>
                        ...
                        {adminMembers.map((m) => (
                          ...
                          >
                            <div className="flex items-center gap-3 min-w-0">
                              <Avatar email={m.email} size={26} position={m.position} />

                              <div className="min-w-0">
                                <div className="text-[13px] font-semibold text-gray-900 truncate">
                                  {m.name || "(이름 없음)"}
                                </div>
                                <div className="text-[12px] text-gray-500 truncate">{m.username || "—"}</div>
                              </div>
                            </div>
          ...
          {editOpen && editItem && (
            ...
            >
              <div
                className="w-full h-full bg-white rounded-none shadow-2xl border border-gray-100 overflow-hidden"
